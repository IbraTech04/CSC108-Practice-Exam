...
jtuwrdubvbywhrrcjyalhrnllfrnsaqelgpv
    ...
    \clearpage
    \question[10] \textbf{Malware Containment I} \\
    ...
    \end{lstlisting}
    \clearpage
    \question[10] \textbf{Malware Containment II} \\
    \texttt{Themba.java} has caught wind of the rogue TA infecting his UltraSheet. He has decided to implement a new way to distribute the UltraSheet. Instead of sending all the parts of the textbook to each user, he will send each user a single part of the textbook. Each user will then send their part to another user. This process will repeat until all users have all parts of the textbook. Write an algorithm to determine whether an arbitrary starting point can result in having zero malware infections. 
    \end{questions}
